...
def power_kmeans_bregman(phi, X, s, k, centers, n_epochs=1, lr=1e-2, iterative=False, n_iters=100, y=None, i=0):
    start = time.time()
    ...
    while True:
        ...
        bregman_dist = pairwise_bregman(X, centers, phi)#torch.clamp(phi[0](X)[:, np.newaxis] - phi[0](centers)[np.newaxis, :] - torch.sum((X[:, np.newaxis] - centers[np.newaxis, :]) * phi[1](centers[np.newaxis, :]), axis=-1), min=1e-12, max=1e12)
        ...
        classes = torch.argmin(pairwise_bregman(X, centers, phi), axis=1)
